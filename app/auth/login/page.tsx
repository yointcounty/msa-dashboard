...
export default function LoginPage() {
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) { e.preventDefault(); setBusy(true); setStatus(''); const data = new FormData(e.currentTarget); try { await signIn(String(data.get('email')), String(data.get('password'))); router.push('/dashboard'); } catch (error) { setStatus(error instanceof Error ? error.message : 'Unable to sign in.'); setBusy(false); } }
